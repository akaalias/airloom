...
// pressure -- own telemetry data per row (the streamline and pressure
// viewers never share flight data), but ONE shared camera across BOTH
// rows, so dragging any tile in either row orbits every tile on the
// page, not just the rest of its own row. Created lazily by whichever
// row mounts first; not tied to either row's own mount/unmount cycle,
// so orbit position survives both rows scrolling off-screen and back.
var sharedRowState=null;
// WebGL contexts are a scarce, capped resource on mobile
...
function startScenarioRow(rowId,mode){
  ...
  function tick(ts){
    ...
    pst.redraw(); // one state shared across both rows: draws every box everywhere
  }
  ...
  function mount(){
    ...
    pending=true;
    // low chase-cam pitch; shared across BOTH rows, not just this one
    // (see sharedRowState above) -- created once, by whichever row
    // mounts first
    if(!sharedRowState)sharedRowState=AL.makeState(0.35);
    pst=sharedRowState;
