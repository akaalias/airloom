...
function flOpen(scen){
  ...
  FL.wantScen=scen;
  AL.ensureFlowLines(FL.hash,scen).then(function(d){
    if(flV&&FL.wantScen===scen)flV.setFlowLines(d)});
  if(FLB.hash)AL.ensureFlowLines(FLB.hash,scen).then(function(d){
    if(flVB)flVB.setFlowLines(d)});
  else if(flVB)flVB.setFlowLines(null);
