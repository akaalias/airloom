...
GH_RIBBON_HTML = (
    ...
    "@media(max-width:600px){#gh-ribbon{right:-50px;bottom:24px;"
    "font-size:9px;letter-spacing:.09em;padding:5px 44px}}</style>"
...
/* generic lazy-load states: any element that fetches its own mesh/
   telemetry off a scroll observer gets one of these -- a spinner while
   in flight, a plain fallback note if a context genuinely can't be had
   (WebGL contexts are a capped, scarce resource on mobile). Self-
   contained (sets its own position:relative), so callers just toggle
   the class on whatever wrapper element makes sense for them. */
@keyframes al-spin{to{transform:rotate(360deg)}}
.al-loading{position:relative}
.al-loading::after{content:"";position:absolute;top:50%;left:50%;
  width:26px;height:26px;margin:-13px 0 0 -13px;
  border:2px solid var(--rule);border-top-color:var(--muted);
  border-radius:50%;animation:al-spin .8s linear infinite;z-index:5}
.al-failed{position:relative}
.al-failed::after{content:"3D unavailable";position:absolute;top:50%;
  left:50%;transform:translate(-50%,-50%);font:11px var(--mono);
  color:var(--faint);white-space:nowrap;z-index:5}
...
/* the full-screen overlay (drag-to-orbit split panes, tab switching)
   isn't a good mobile experience, and opening it can add up to 8 more
   WebGL contexts on top of whatever's already live on the page -- real
   pressure on a small mobile GPU budget. Most of what it offers is
   already on the page as live tiles, so just don't offer the door in. */
@media(max-width:600px){.vbtns{display:none}}
...
/* mobile: the viewer/dmeta/parents row's min-widths (300+280+170px, plus
   gaps) far exceed a phone's viewport and force horizontal scrolling of
   the whole page -- stack them instead, full width, same as .genrow and
   .dcols2 already do at their own breakpoints */
@media(max-width:820px){
  .detail .dcols{flex-direction:column;gap:18px}
  .viewer{width:100%;min-width:0}
  .dmeta{min-width:0}
  .parents{width:100%;display:flex;gap:14px;flex-wrap:wrap}
  .parents figure{flex:1;min-width:140px;margin:0}
}
...
/* ...and swaps the part-color legend for the mono two-tone one */
.ovl-lgd .mono-lgd{display:none}
#ovl.perf .ovl-lgd .lgd:not(.mono-lgd){display:none}
#ovl.perf .ovl-lgd .mono-lgd{display:flex}
...
function flPoseData(d,V,S,dtf){
  ...
  // the out-and-back mission reverses ground-track heading by exactly
  // 180 degrees at the turnaround (and again at the loop restart) --
  // a straight-line reciprocating path, not a curve, so the raw
  // position-derivative heading flips sign in a single sample. Turning
  // that into a hard instantaneous snap read as a bug (and looked like
  // it was caused by whatever the viewer was doing at that moment, since
  // it has no visibility into the telemetry clock). Slew-limit the
  // heading ANGLE instead of copying the raw vector, so a reversal reads
  // as a fast continuous turn.
  if(hm>=1e-4){
...
function flShow(dtf){
  ...
  // wind channel: streamlines of the relative wind each pane is flying
  if(flV)flV.windUpdate([p.L("wx"),p.L("wy"),p.L("wz")],dtf||0);
  if(flVB)flVB.windUpdate(pb?[pb.L("wx"),pb.L("wy"),pb.L("wz")]:[0,0,0],
                          dtf||0);
  flState.redraw();
...
function flbLoad(){ // the ancestor mesh is scenario-independent: load once
  ...
  if(FLB.loaded!==FLB.hash){
    flVB.load([{id:"m-"+FLB.hash,propSpin:true,mono:true}]);
    FLB.loaded=FLB.hash;
  }
  flbLab();
}
...
function flOpen(scen){
  ...
  // real CFD streamlines where a payload exists (falls back to the
  // analytic field otherwise); wantScen guards a quick tab switch.
  // The lines anchor at the trace's MEAN attitude (fd needs pd), so
  // the wind stays world-fixed while the craft oscillates within it.
  FL.wantScen=scen;
  var flowA=AL.ensureFlowLines(FL.hash,scen);
  var flowB=FLB.hash?AL.ensureFlowLines(FLB.hash,scen)
                    :Promise.resolve(null);
  if(!FLB.hash&&flVB)flVB.setFlowLines(null);
  var jobs=[ensureFlight(FL.hash,scen)];
  if(FLB.hash)jobs.push(ensureFlight(FLB.hash,scen));
  Promise.all(jobs).then(function(){
    var pd=FLIGHTS[FL.hash+"|"+scen];
    if(!pd)return;
    flowA.then(function(fd){
      if(flV&&FL.wantScen===scen)
        flV.setFlowLines(fd,fd?AL.meanPose(pd):null)});
    var pdb=FLB.hash?FLIGHTS[FLB.hash+"|"+scen]:null;
    flowB.then(function(fd){
      if(flVB&&FL.wantScen===scen)
        flVB.setFlowLines(fd,fd&&pdb?AL.meanPose(pdb):null)});
    ...
    flV.load([{id:"m-"+FL.hash,propSpin:true,mono:true}]);
    FL.theta=0;
...
// Pages copy is a frozen report where nothing new can fly in. Pages
// that opt out (the landing: it would reset the replay and tree pin)
// set NO_LIVE_RELOAD before this script.
if(!window.NO_LIVE_RELOAD&&
...
def overlay_html() -> str:
    ...
    return (
        ...
        "</div>"
        f'<div class="ovl-lgd">{_parts_legend_html()}'
        '<div class="lgd mono-lgd">'
        '<span class="lg"><span class="sw">'
        '<span class="dot" style="background:#6b6960"></span></span>'
        "evolved parts (deck + arms)</span>"
        '<span class="lg"><span class="sw">'
        '<span class="dot" style="background:#c2c0b8"></span></span>'
        "fixed kit</span>"
        '<span class="lg"><span class="sw">'
        '<span class="dot" style="background:#2e6e63"></span></span>'
        "airflow (RANS streamlines)</span>"
        "</div></div>"
