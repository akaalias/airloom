...
.hit{cursor:pointer}
svg.focus .nd:not(.lit),svg.focus .bs:not(.lit),svg.focus .ch:not(.lit),
svg.focus .cl:not(.lit),svg.focus .im:not(.lit),
svg.focus .xm:not(.lit){opacity:.45}
svg.focus .ed:not(.lit),svg.focus .el:not(.lit){opacity:.12}
...
// only a click INSIDE the tree charts releases a pin: dragging a 3D
// model or pressing play elsewhere on the page must not clear the
// highlighted lineage (node clicks stopPropagation, so they re-pin)
document.querySelectorAll(".trees").forEach(function(t){
  t.addEventListener("click",unpin);
});
